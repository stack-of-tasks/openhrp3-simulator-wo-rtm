...
//			if(!(IR(d0)&SIGN_BITMASK) && !(IR(d1)&SIGN_BITMASK) && !(IR(d2)&SIGN_BITMASK))	return FALSE;
                        if (collisionPairInserter){
                            hrpGep::CollisionPairInserterBase &c = *collisionPairInserter;
                            hrpGep::collision_data cdata;
                            cdata.num_of_i_points = 2;
                            cdata.i_point_new[0] = cdata.i_point_new[1] = 1;
                            cdata.i_point_new[2] = cdata.i_point_new[3] = 0;
                            hrpGep::Vector3 relN; 
                            hrpGep::getVector3(relN, p->n);
                            ...
                            if (d0>0||d1>0||d2>0){
                                hrpGep::Vector3 v1, v2;
                                hrpGep::getVector3(v1, p1);
                                hrpGep::getVector3(v2, p2);

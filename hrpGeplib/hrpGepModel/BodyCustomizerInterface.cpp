...
/**
   ...
   hrpGep::findBodyCustomizer() function.

   \param pathString the path to a DLL file or a directory that contains DLLs
   \param bodyInterface
*/
int hrpGep::loadBodyCustomizers(const std::string pathString, BodyInterface* bodyInterface)
{
...
int hrpGep::loadBodyCustomizers(const std::string pathString)
{
...
#ifndef _WIN32
        Dl_info info;
        if(dladdr((void*)&hrpGep::findBodyCustomizer, &info)){
            filesystem::path customizerPath =
                filesystem::path(info.dli_fname).branch_path().branch_path() / OPENHRPGEP_RELATIVE_SHARE_DIR / "customizer";
            numLoaded += loadBodyCustomizers(customizerPath.string(), bodyInterface);
        }
#else
        string customizerPath(OPENHRPGEP_SHARE_DIR);
...
int hrpGep::loadBodyCustomizers()
{
    return loadBodyCustomizers(Body::bodyInterface());
}


BodyCustomizerInterface* hrpGep::findBodyCustomizer(std::string modelName)
{
    BodyCustomizerInterface* customizerInterface = 0;

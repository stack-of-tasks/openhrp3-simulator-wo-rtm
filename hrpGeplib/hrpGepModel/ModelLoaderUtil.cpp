...
#include <hrpGepCorba/ViewSimulator.hh>
#include <hrpGepCollision/ColdetModel.h>
#include <stack>
...
bool hrpGep::loadBodyFromBodyInfo(BodyPtr body, OpenHRP::BodyInfo_ptr bodyInfo, bool loadGeometryForCollisionDetection, Link *(*f)())
{
...
BodyInfo_var hrpGep::loadBodyInfo(const char* url, int& argc, char* argv[])
{
    CORBA::ORB_var orb = CORBA::ORB_init(argc, argv);
    return loadBodyInfo(url, orb);
}

BodyInfo_var hrpGep::loadBodyInfo(const char* url, CORBA_ORB_var orb)
{
...
ModelLoader_var hrpGep::getModelLoader(CORBA_ORB_var orb)
{
    CosNaming::NamingContext_var cxt;
    try {
        CORBA::Object_var nS = orb->resolve_initial_references("NameService");
...
ModelLoader_var hrpGep::getModelLoader(CosNaming::NamingContext_var cxt)
{
...
BodyInfo_var hrpGep::loadBodyInfo(const char* url, CosNaming::NamingContext_var cxt)
{
...
bool hrpGep::loadBodyFromModelLoader(BodyPtr body, const char* url, CosNaming::NamingContext_var cxt,  bool loadGeometryForCollisionDetection)
{
...
bool hrpGep::loadBodyFromModelLoader(BodyPtr body, const char* url, CORBA_ORB_var orb, bool loadGeometryForCollisionDetection)
{
    CosNaming::NamingContext_var cxt;
    try {
        CORBA::Object_var nS = orb->resolve_initial_references("NameService");
...
bool hrpGep::loadBodyFromModelLoader(BodyPtr body, const char* url, int& argc, char* argv[], bool loadGeometryForCollisionDetection)
{
    CORBA::ORB_var orb = CORBA::ORB_init(argc, argv);
